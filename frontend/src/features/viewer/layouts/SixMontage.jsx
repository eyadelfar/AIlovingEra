  ...
  return (
    <PageShell style={style} className={`${style.innerPadding} flex flex-col`}>
      <div className="relative z-20 flex flex-col h-full gap-2">
        {/* Row 1: wide + narrow */}
        <div className="flex gap-2 overflow-hidden flex-[2.8] min-h-0">
        ...
        {/* Row 3: equal pair */}
        <div className="flex gap-2 overflow-hidden flex-[2.4] min-h-0">
        ...
        <div className="flex-1 flex items-center justify-center min-h-0">
          {page.heading_text && <h3 data-ts="heading" className={`font-semibold ${style.heading} mr-3`}>{page.heading_text}</h3>}
          {page.body_text && <p data-ts="body" className={`${style.body} text-xs line-clamp-1`}>{page.body_text}</p>}
        </div>
      </div>
    </PageShell>
  );
