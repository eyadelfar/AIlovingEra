...
export default function SignupPage() {
  ...
  // Store referral code for OAuth flow
  useEffect(() => {
    if (refCode) {
      localStorage.setItem('keepsqueak_ref', refCode);
    }
  }, [refCode]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const pendingRef = refCode || localStorage.getItem('keepsqueak_ref');
      if (pendingRef && data.session) {
        try {
      ...
      setError(err.message || t('failedToSignUp'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleGoogle = async () => {
    ...
    try {
      ...
      setError(err.message || t('failedToSignInWithGoogle'));
    }
  };
        ...
        <div className="w-full max-w-md text-center">
          ...
          <div className="mt-8 bg-gray-900 border border-gray-800 rounded-2xl p-8">
            ...
            <p className="text-gray-400 text-sm">
              <Trans i18nKey="auth:confirmationEmailSent" values={{ email }}>
                We sent a confirmation link to <strong>{{ email }}</strong>. Click the link to activate your account.
              </Trans>
            </p>
            <Link to="/login" className="mt-6 inline-block text-rose-400 hover:text-rose-300 text-sm">
              {t('backToLogin')}
            </Link>
          </div>
        </div>
    ...
    <div className="min-h-screen flex items-center justify-center px-4 bg-gray-950">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-400 mt-2">{t('createYourAccount')}</p>
        </div>
